...
dir = "results_mult_freq_50000_points"
...
for p_idx, p in enumerate(precentage_of_missed_data):
    ...
    n_points = int(50000 / (1 - p))  # Адаптивный размер
    ...
    for run in range(repetition_num):
        ...
        try:
            ...
            if run == 0:
                ...
                plt.tight_layout()
                plt.savefig(f'{dir}/plot_{int(p*100):02d}_run{run:02d}.png', dpi=100)
                plt.close()
...
plt.xlim(-5, 105)
plt.tight_layout()
plt.savefig(f'{dir}/rmse_vs_missing.png', dpi=120)
plt.close()

print("\nАнализ завершен. Результаты сохранены в папке 'results_mult_freq_5000_points'")